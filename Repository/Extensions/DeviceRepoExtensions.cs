...
namespace Repository.Extensions
{
    ...
    {
        public static IQueryable<Device> Search(this IQueryable<Device> devices, string? searchTerm, string? options)
        {
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                ...
                {
                    case "mac":
                        return devices.Where(d => d.MAC.Contains(searchTerm));
                        
                    case "domain":
                        return devices.Where(d => d.DomainIDIfExists.Contains(searchTerm));
                        
                    default:
                        return devices.Where(d => d.MAC.Contains(searchTerm) || d.Owner.Contains(searchTerm) || d.DomainIDIfExists.Contains(searchTerm));
